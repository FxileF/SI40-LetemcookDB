...
-- Procédures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `addComment` (IN `body` TEXT, IN `idParent` BIGINT, IN `userID` BIGINT, OUT `postID` BIGINT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `addIngredient` (IN `name` VARCHAR(100), OUT `ingredientID` BIGINT)   BEGIN
    INSERT INTO ingredient (NAME)
    VALUES (name);
	SET ingredientID = LAST_INSERT_ID();
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `addInstruction` (IN `postID` BIGINT, IN `body` TEXT, IN `instructionOrder` INT, OUT `instructionID` INT)  MODIFIES SQL DATA BEGIN
	INSERT INTO instructions (IDPOST,BODY,INSTRUCTIONORDER)
    VALUES (postID,body,instructionOrder);
	SET instructionID = LAST_INSERT_ID();
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `addRecipe` (IN `userID` BIGINT, IN `title` TEXT, IN `description` TEXT, IN `imageExtension` VARCHAR(50), IN `difficulty` BIGINT, IN `duration` TIME(1), IN `nbPeople` BIGINT, IN `category` VARCHAR(50), IN `type` VARCHAR(50), OUT `postID` BIGINT)  MODIFIES SQL DATA BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateRecipe` (IN `postID` BIGINT, IN `title` TEXT, IN `description` TEXT, IN `difficulty` BIGINT, IN `duration` TIME(1), IN `nbPeople` BIGINT, IN `category` VARCHAR(50), IN `type` VARCHAR(50))  MODIFIES SQL DATA BEGIN
...
CREATE TABLE `follows` (
  `USERID_FOLLOWER` BIGINT NOT NULL,
...
CREATE TABLE `instructions` (
  `IDINSTRUCTION` BIGINT NOT NULL,
  `IDPOST` BIGINT NOT NULL,
  `BODY` text DEFAULT NULL,
  `INSTRUCTIONORDER` BIGINT DEFAULT NULL,
...
--
ALTER TABLE `ban`
  MODIFY `BANID` BIGINT NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- AUTO_INCREMENT pour la table `ingredient`
--
ALTER TABLE `ingredient`
  MODIFY `IDINGREDIENT` BIGINT NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=63;

--
-- AUTO_INCREMENT pour la table `instructions`
--
ALTER TABLE `instructions`
  MODIFY `IDINSTRUCTION` BIGINT NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=76;

--
-- AUTO_INCREMENT pour la table `notification`
--
ALTER TABLE `notification`
  MODIFY `IDNOTIF` BIGINT NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=29;

--
-- AUTO_INCREMENT pour la table `post`
--
ALTER TABLE `post`
  MODIFY `IDPOST` BIGINT NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=117;

--
-- AUTO_INCREMENT pour la table `user`
--
ALTER TABLE `user`
  MODIFY `USERID` BIGINT NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
